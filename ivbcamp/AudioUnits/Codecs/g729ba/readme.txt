...
TITLE
-----
Fixed-point description of Recommendation G.729A with ANNEX B
...
per Corrigendum to annex B of G.729 published in COM 16-R20.
...
file : DEC_LD8A.C
*****************
Version 1.2 lines 151 to 153 :
----------------------------------------------------------------------------
  if(bfi == 1)
    if(past_ftyp == 1) ftyp = 1;
    else ftyp = 0;
----------------------------------------------------------------------------
replaced in Version 1.3 by lines 151 to 155 :
----------------------------------------------------------------------------
  if(bfi == 1) {
    if(past_ftyp == 1) ftyp = 1;
    else ftyp = 0;
...
are replaced by (lines 193-205 and 264-276) :
---------------------------------------------------------
  for (q=0; q<K; q++){
    min_indx_p[q] = 0;
    min_indx_m[q] = 0;
...
of the G.729A codec with VAD/DTX/CNG as described in ANNEX B.

 It includes also the PC executable (coder.exe and decoder.exe), a batch file
(test.bat), speech (test.inp) and data files (test_a.bit and test_a.syn) to 
verify the execution.  The binary reference files are in PC format. 


SIMILARITIES AND DIFFERENCES WITH G.729A
----------------------------------------

Common files with G.729A
~~~~~~~~~~~~~~~~~~~~~~~~
acelp_ca.c
basic_op.c
cor_func.c
...
Files in G.729A but modified for ANNEX B
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
bits.c
cod_ld8a.c
coder.c
