  ...
  *-------------------------------------------------------------------*/
  ...
  for(i0=0; i0<L_SUBFR; i0+=STEP)
  {
    ...
    for(i1=1; i1<L_SUBFR; i1+=STEP)
    {
      ...
      *p0++ = mult(pp0, psign[i1]);
      *p1++ = mult(pp1, psign[i1+1]);
      *p2++ = mult(pp2, psign[i1+2]);
      *p3++ = mult(pp3, psign[i1+3]);
    }
  }
  ...
  for(i1=1; i1<L_SUBFR; i1+=STEP)
  {
    ...
    for(i2=2; i2<L_SUBFR; i2+=STEP)
    {
        ...
        Word16 pp1 = *p1;
        Word16 pp2 = *p2;
      *p0++ = mult(pp0, psign[i2]);
      *p1++ = mult(pp1, psign[i2+1]);
      *p2++ = mult(pp2, psign[i2+2]);
    }
  }
  ...
  for(i2=2; i2<L_SUBFR; i2+=STEP)
  {
    ...
    for(i3=3; i3<L_SUBFR; i3+=STEP)
    {
        Word16 pp0 = *p0;
        Word16 pp1 = *p1;
      *p0++ = mult(pp0, psign[i3]);
      *p1++ = mult(pp1, psign[i3+1]);
    }
  }
